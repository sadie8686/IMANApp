...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        // Create Cell
        CollapseClickCell *cell = [CollapseClickCell newCollapseClickCellWithTitle:[CollapseClickDelegate titleForCollapseClickAtIndex:xx] subTitle:[CollapseClickDelegate subtitleForCollapseClickAtIndex:xx] index:xx content:[CollapseClickDelegate viewForCollapseClickContentViewAtIndex:xx]];
        ...
        // Set cell.SubTitleLabel's Color
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForSubTitleLabelAtIndex:)]) {
            cell.subTitleLabel.textColor = [CollapseClickDelegate colorForSubTitleLabelAtIndex:xx];
        }
        else {
...
-(void)didSelectCollapseClickButton:(UIButton *)titleButton {
    ...
    // Cell is CLOSED -> OPEN
        ...
        for (int i=0; i<self.dataArray.count; i++) {
            [self closeCollapseClickCellAtIndex:i animated:NO];
        }
        [self openCollapseClickCellAtIndex:titleButton.tag animated:YES];
        NSString *tag = [NSString stringWithFormat:@"%d", titleButton.tag];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"StoreTappedView" object:tag];
    }
}
...
-(void)openCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] != YES) {
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            cell.TitleView.backgroundColor = [UIColor colorWithRed:0.0/255.0 green:204.0/255.0 blue:255.0/255.0 alpha:1.0];
            cell.TitleLabel.textColor = [UIColor whiteColor];
            cell.subTitleLabel.textColor = [UIColor whiteColor];
...
-(void)closeCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] == YES) {
        ...
        [UIView animateWithDuration:duration animations:^{
            ...
            if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForCollapseClickTitleViewAtIndex:)]) {
                cell.TitleView.backgroundColor = [CollapseClickDelegate colorForCollapseClickTitleViewAtIndex:index];
            }
            else {
                cell.TitleView.backgroundColor = [UIColor colorWithWhite:0.4 alpha:1.0];
            }
            ...
            // Set cell.SubTitleLabel's Color
            if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForSubTitleLabelAtIndex:)]) {
                cell.subTitleLabel.textColor = [CollapseClickDelegate colorForSubTitleLabelAtIndex:index];
            }
            else {
                cell.subTitleLabel.textColor = [UIColor whiteColor];
            }
